...
v2.2.0 (2026-07-23) — Prose Style Scanner + Anti-Measurement Pack
  · A24 설정 재료 반복 상한 룰 신설
    (버릇·습관·말버릇·상징 소품의 회차당 출현 상한 명시)
  · 캐릭터 바이블 프롬프트에 버릇 발동 조건 규칙 추가
  · REDO 프롬프트에 prose_scan_report 주입 파라미터 추가
  · 품질 체크 검사 항목 A1~A24로 갱신
  · engine_validator.py 연동: 문체 정적 스캐너 신설

v2.1.8 (2026-07-23) — Anti-Measurement Prose + DOCX Layout Fix
  · A23 계측·관찰 리포트체 금지 룰 신설
    (초 단위 수치·각도 분석·인지 부정 부연·명사문 신원 삽입 금지)
  · 품질 체크 프롬프트 검사 항목 A1~A23으로 갱신
  · docx_builder.py 연동: 회차 DOCX 메타 정보 라인 제거,
    빈 단락 제거 + 행간 1.5 (전남편 검토용 서식 기준)
...
# ═══════════════════════════════════════════════════
# AI 문체 탈출 규칙 A1~A24 (무협 특화)
# v2.1.0: A21 1차/2차 위기 구조 + A22 첫 장면 9유형 추가
# v2.2.0: A23 계측·관찰 리포트체 금지 + A24 설정 재료 반복 상한 추가
# ═══════════════════════════════════════════════════

AI_ANTI_PATTERN = """[AI 문체 탈출 규칙 — 무협 A1~A24]
...
## 문체 룰 (A23~A24) ⭐⭐ — v2.2.0 신규

A23. 계측·관찰 리포트체 금지
  서술자가 계측기·분석 카메라처럼 수치와 각도를 기록하는 문체는
  AI 문체의 대표 흔적이다. 다음을 전면 금지한다.
  - 시간 계측 서술 금지: "0.1초", "이 초쯤", "초당 세 곳",
    "삼 초 만에" 같은 초 단위 수치 표기. 속도감은 수치가 아니라
    감각과 결과로 표현한다.
    (X) 콧등을 지나 검첨까지. 0.1초.
    (O) 콧등을 스친 바람이 검끝에 닿기도 전이었다.
  - 각도·자세 분석 서술 금지: "각도는 ~ 자세였다", "어깨선",
    "발끝의 방향" 같은 동작 분해 리포트. 동작은 분해하지 말고
    한 호흡의 이미지로 보여준다.
  - 인지 부정 부연 금지: "본인은 그것을 의식하지 못했다",
    "자신은 느끼지 못했다" 같은 서술자의 계측 보고.
    인물이 모르는 것은 독자도 장면으로만 감지하게 한다.
  - 명사문 신원 삽입 금지: "얼굴은 최형근." 처럼 명사 종결
    단문으로 정보를 끼워 넣지 말 것. 이름·신원은 행동이나
    대사 속에서 자연스럽게 드러낸다. (A9의 명사 끊기는 리듬
    기법이지 정보 전달 수단이 아니다.)
  - "아주 미세하게", "미묘하게" 같은 정밀 관찰 부사 남용 금지.

A24. 설정 재료 반복 상한 (버릇·말버릇·습관·소지품)
  캐릭터에 설정된 버릇·습관·말버릇·상징 소품은 "존재 증명"이
  아니라 "필요할 때 꺼내는 카드"다. 설정을 매 장면 시연하는 것은
  AI 문체의 대표 흔적이다.
  - 설정된 버릇·습관의 실행 묘사: 회차당 최대 1~2회.
    플롯 기능이나 감정 전환에 복무할 때만 꺼낼 것. 기능이
    없으면 0회가 정답이다.
    (X) 메모 습관 설정 → 모든 행동 끝에 메모하는 묘사
    (O) 결정적 순간 1회, 메모가 서사를 움직이는 배치
  - 말버릇·습관 멘트(speech_samples의 반복 대사 포함):
    회차당 1회 이하. 매 대화마다 반복 금지.
  - 상징 소품(구슬·붉은 끈·손목시계 등) 확인 동작:
    회차당 최대 2회. 매 장면 소품을 만지작거리게 하지 말 것.
  - 같은 표현의 재등장은 간격을 벌릴수록 강해진다.
    반복은 리듬이 아니라 마모다.
"""
...
## 버릇·습관 설정 규칙 (A24 연동)
- 버릇·습관·반복 멘트를 설정할 때는 반드시 **발동 조건**을 함께 명시
  (예: "메모 습관 — 단, 감정이 흔들릴 때만 수첩을 꺼낸다")
- 발동 조건 없는 버릇은 집필 시 매 장면 반복되는 원인이 된다
- 상징 소품에도 동일 적용 (예: "구슬 — 위기 감지 시에만 반응")
...
5. 한자 병기 규칙 (첫 등장만)
6. 사자성어 남발
7. AI 문체 흔적 (A1~A24) — 특히 A23 계측·리포트체, A24 버릇 반복
...
def build_episode_redo_prompt(
    ...
    Args:
        ...
        prose_scan_report: 문체 스캐너 검출 리포트 (v2.2.0 — 있으면 주입)
    """
    weak_axes = []
...
{f'''### 문체 스캐너 검출 (아래 문장들의 패턴을 반드시 제거 — A23·A24)
{prose_scan_report}
검출된 문장을 단순 삭제하지 말고, 같은 정보를 감각·행동·결과로 다시 쓸 것.''' if prose_scan_report else ''}
